...
from DevuMusic.Helpers.inline import (
    buttons,
    close_key,
    help_back,
    helpmenu,
    ...
    if CallbackQuery.from_user.id != int(user_id):
        try:
            return await CallbackQuery.answer(
                "It's better if you stay in your limits.", show_alert=True
            )
        except:
            return
...
async def admin_cbs(_, query: CallbackQuery):
    ...
    if data == "resume_cb":
        ...
        get = devudb.get(query.message.chat.id)
        if not get:
            try:
                await _clear_(query.message.chat.id)
                await pytgcalls.leave_group_call(query.message.chat.id)
                await query.message.reply_text(
                    text=f"Stream skipped.\nBy: {query.from_user.mention}\n\nNo more queued tracks in {query.message.chat.title}, leaving voice chat.",
                    reply_markup=close_key,
                )
...
@app.on_callback_query(filters.regex("devu_help"))
async def help_menu(_, query: CallbackQuery):
    ...
    try:
        await query.edit_message_text(
            text=f"Hey {query.from_user.first_name},\n\nPlease click on the button below for the help menu.",
...
@app.on_callback_query(filters.regex("devu_cb"))
...
@app.on_callback_query(filters.regex("devu_home"))
async def home_devu(_, query: CallbackQuery):
    try:
        await query.answer()
    except:
        pass
